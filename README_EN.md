...
<b> Automatically extract charts from attachments, upload any materials, and propose modifications verbally—moving towards a true "Vibe PPT." </b>

<b>🎯 Lowering the barrier to PPT creation, enabling everyone to quickly create beautiful and professional presentations.</b>

<br>

*If this project is useful to you, feel free to star🌟 & fork🍴*
...
Have you ever found yourself in this dilemma: the presentation is due tomorrow, yet your PPT remains a blank canvas; your mind is brimming with brilliant ideas, but your passion is drained by tedious layout and design?
...
- 2️⃣ Low degree of freedom makes it difficult to perform multiple rounds of revisions.
- 3️⃣ The final products look similar, suffering from severe homogenization.
- 4️⃣ Low-quality assets that lack specificity.
- 5️⃣ Fragmented text and image layouts with poor design aesthetics.

These shortcomings make it difficult for traditional AI PPT generators to simultaneously satisfy our two core needs: "speed" and "aesthetics." Even those claiming to be "Vibe PPT" are, in my eyes, still far from being truly "Vibe."

However, the emergence of the nano banana🍌 model has changed everything. I tried using 🍌pro to generate PPT pages and found that the results were exceptional in terms of quality, aesthetics, and consistency. Moreover, it can accurately render almost all text requested in the prompts while strictly following the style of reference images. So, why not build a native "Vibe PPT" application based on 🍌pro?

## 👨‍💻 Use Cases

1. **Beginners**: Quickly generate aesthetic PPTs with zero barriers and no design experience required, reducing the hassle of choosing templates.
2. **PPT Professionals**: Refer to AI-generated layouts and combinations of graphics and text to quickly gain design inspiration.
3. **Educators**: Quickly convert teaching materials into lesson plan PPTs with visual aids to enhance classroom effectiveness.
4. **Students**: Rapidly complete assignment presentations, allowing focus on content rather than formatting and aesthetics.
...
| **R&D and Industrialization of Intelligent Equipment for Prepared Meal Production Lines** | **The Evolution of Money: A Journey from Shells to Banknotes** |
...
- **Natural Language Editing**: Supports modifying outlines or descriptions via "Vibe" prompts (e.g., "Change the third page to a case study"), with AI responding and adjusting in real-time.
...
- **Multi-format Support**: Upload files such as PDF, Docx, MD, Txt, etc., and the backend will automatically parse the content.
...
- **Inpainting**: Perform verbal edits on unsatisfactory areas (e.g., "change this image to a pie chart").
...
- **If the editable PPT results are poor, such as overlapping text or unstyled text, it is usually due to configuration issues. Please refer to [Editable PPTX Export FAQ and Troubleshooting Solutions](https://github.com/Anionex/banana-slides/issues/121#issuecomment-3708872527) for troubleshooting.**
...
| --- | --- | --- |
| Max Page Limit | 15 pages | **Unlimited** | 
| Post-editing | Not supported | **Selection Editing + Chat-based Editing** |
| Adding Assets | Cannot add after generation | **Freely add after generation** |
| Export Formats | Supports PDF export only | **Export as PDF, (editable) PPTX** |
| Watermarks | Watermark in free version | **No watermark, freely add/remove elements** |

> Note: As new features are added, this comparison may become outdated.

## 🔥 Recent Updates

- [Jan 4th]: v0.3.0 Released: Major upgrade to editable PPTX export:
  ...
  * **For detailed effects and usage instructions, see https://github.com/Anionex/banana-slides/issues/121**

- [Dec 27th]: Added support for "no-image template" mode and high-quality text presets; PPT page styles can now be controlled via pure text descriptions.
- [Dec 24th]: Added editable PPTX export based on nano-banana-pro background extraction to the main branch (currently in Beta)

## **Frequently Asked Questions**

1. **Garbled text or blurry text on generated pages**
    - You can choose a higher output resolution (OpenAI format may not support increasing resolution; Gemini format is recommended). Based on testing, increasing the resolution from 1k to 2k before generating pages significantly improves text rendering quality.
    - Please ensure that the page description includes the specific text content to be rendered.

2. **Poor results when exporting editable PPT, such as overlapping text, missing styles, etc.**
...
4. **Receiving 503 Error or Retry Error when generating content**
    - You can check the Docker backend logs using the commands provided in the README to locate the detailed error for the 503 issue. This is usually caused by incorrect model configuration.
...
5. **Why is the API Key set in .env not taking effect?**
    ...
    - If parameters were previously configured on the web settings page, they will override the parameters in `.env`. You can restore the `.env` settings by clicking "Restore Default Settings".
...
| ✅ Completed | Area selection and Vibe-style verbal editing for single slides |
| ✅ Completed | Asset module: asset generation, uploading, etc. |
| ✅ Completed | Support for uploading and parsing multiple file types |
| ✅ Completed | Support Vibe-style verbal adjustment of outlines and descriptions |
| ✅ Completed | Preliminary support for exporting editable PPTX files |
| 🔄 In Progress | Support for exporting editable PPTX with multi-layering and precise matting |
...
> **The LLM interface in this project follows the AIHubMix platform standard. It is recommended to use [AIHubMix](https://aihubmix.com/?aff=17EC) to obtain an API key to reduce migration costs.**  
```env
...
# Gemini Configuration Format (Used when AI_PROVIDER_FORMAT=gemini)
...
# GCP Service Account Required; GCP Free Tier Available
...
**Use the new editable export configuration method to achieve better editable export results**: You need to obtain an API KEY from the [Baidu AI Cloud Platform](https://console.bce.baidu.com/iam/#/iam/apikey/list) and fill it in the `BAIDU_OCR_API_KEY` field in the `.env` file (there is a generous free usage quota). For details, see the instructions in https://github.com/Anionex/banana-slides/issues/121

<details>
  <summary>📒 Using Vertex AI (GCP Free Credit)</summary>
...
> **Note**: For the `gemini-3-*` series models, you need to set `VERTEX_LOCATION=global`
...
The project provides pre-built frontend and backend images on Docker Hub (synced with the latest version of the main branch), allowing you to skip the local build process for rapid deployment:
...
Pull the latest code and rebuild and start the services:
...
> **The project's LLM interfaces follow the AIHubMix platform format. It is recommended to use [AIHubMix](https://aihubmix.com/?aff=17EC) to obtain API keys to reduce migration costs.** 
```env
...
│   │   │   └── History.tsx          # Version history management page
...
│   │   ├── task.py             # Task model (Async tasks)
...
To facilitate communication and mutual support, this WeChat group has been established.

New feature suggestions or feedback are welcome. I will also answer questions in a ~~laid-back~~ manner.
...
The open-source license of this project is non-commercial (CC BY-NC-SA).  
...
> Note: If you have questions about usage scenarios, please contact the author to obtain a license.
...
<details>
  ...
  "Aggregating global multi-model API service providers. Enjoy secure, stable, and 72-hour access to the world's latest models at a lower price."
</details>
...
Thanks to the following friends for their generous sponsorship and support for the project:
> @雅俗共赏、@曹峥、@以年观日、@John、@胡yun星Ethan, @azazo1、@刘聪NLP、@🍟、@苍何、@biubiu  
> If you have any questions regarding the sponsorship list (e.g., your name does not appear after donating), please <a href="mailto:anionex@qq.com">contact the author</a>
